...
def publish_config(mqttc, topic, model, instance, mapping):
    ...
    device_id = "-".join([model, instance])
    object_id = "-".join([device_id, object_suffix])
    ...
    config["device"] = { "name": device_id, "identifiers": device_id, "model": model }
...
def bridge_event_to_hass(mqttc, data):
    ...
    topic = "/".join([MQTT_PREFIX, model, instance, "events"])
...
def rtl_433_bridge():
    ...
    while True:
        ...
        try:
            ...
            bridge_event_to_hass(mqttc, data)
        except json.decoder.JSONDecodeError:
